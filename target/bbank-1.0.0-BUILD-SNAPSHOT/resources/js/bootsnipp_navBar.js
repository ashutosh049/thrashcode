$(document).ready(function() {
	...
	$('#fetchBootArticlesForm').submit(function(){
		...
		var loc = 'http://localhost:8081/poscode/index';
...
function appendArticles(findWithTag) {
	$.ajax({
		...
		dataType : 'json',
		url  : "fetchArticle?tags="+findWithTag,
		success : function(data) {
				...
				$.each(data, function(key,articleList) { 
					...
					$.each(articleList.artcl_tags, function(index, tag) {  
						if(findWithTag!="" && tag.indexOf(findWithTag)>=0){
					...
					"<div class='row'>"+
						"<div class='col-md-12 post-header-line artclInfoLine'>"+
							...
							"by <a href='#' class='artcle_list_anchors'>"+artcl_owner_name+"</a>"+
							"&nbsp;&nbsp;|  "+ 
